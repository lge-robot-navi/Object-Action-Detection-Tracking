...
MSRA in the Imagenet and MSCOCO 2016 competition model ResNet-101 and
...
def resnet_v1(inputs,
  ...
  with variable_scope.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
   ...
   with arg_scope([layers.conv2d, bottleneck,resnet_utils.stack_blocks_dense],outputs_collections=end_points_collection):
...
def resnet_v1_block(scope, base_depth, num_units, stride, mul):
...
def resnet_v1_xception(inputs,
  ...
  blocks = [
      resnet_v1_block('block1', base_depth=24, num_units=4, stride=2, mul=6),
      resnet_v1_block('block2', base_depth=144, num_units=8, stride=2, mul=2),
      resnet_v1_block('block3', base_depth=288, num_units=4, stride=1, mul=2), # stride must be modified
  ]
  return resnet_v1(inputs, blocks, num_classes, is_training,
                   global_pool=global_pool, output_stride=output_stride,
                   include_root_block=True, reuse=reuse, scope=scope)
resnet_v1_xception.default_image_size = resnet_v1.default_image_size
