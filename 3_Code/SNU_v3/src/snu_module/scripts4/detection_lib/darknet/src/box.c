...
void do_nms_sort_custom(detection *dets, int total, int classes, float thresh)
{
    int i, j, k;
    k = total - 1;
    for (i = 0; i <= k; ++i) {
    ...
    int accepted_classes[6] = {0, 1, 2, 3, 5, 7};   // person, bicycle, car, motorbike, bus, truck
    int accepted_classes_idx = 0;


    for (k = 0; k < classes; ++k) {
        if (k != accepted_classes[accepted_classes_idx]) {
            for (i = 0; i < total; i++) {
                dets[i].prob[k] = 0;
            }
            continue;
        }
        accepted_classes_idx++;
        for (i = 0; i < total; ++i) {
            dets[i].sort_class = k;
        }
        qsort(dets, total, sizeof(detection), nms_comparator_v3);
